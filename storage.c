...
storage_write(const char* path, const char* buf, size_t size, off_t offset)
{
    ...
    int tocopy = size;
    while (tocopy > 0) {
        char* dest = pages_get_page(inode_get_pnum(node, nindex));
        dest += nindex % 4096;
        int copied = min(tocopy, 4096 - (nindex % 4096));
        memcpy(dest, buf + bindex, copied);
        bindex += copied;
        nindex += copied;
        tocopy -= copied;
    }
...
storage_mknod(const char* path, int mode)
{
   ...
   // printf("not here\n");
    ...
    printf("***ending mknod and printing inode***\n");
    print_inode(node);
